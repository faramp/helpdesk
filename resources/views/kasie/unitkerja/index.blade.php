
@extends('kasie.template')
@section('title', 'Data Unit Kerja')
@section('content')

  <div class="col g-ml-45 g-ml-0--lg g-pb-65--md">
          <!-- Breadcrumb-v1 -->
          <div class="g-hidden-sm-down g-bg-gray-light-v8 g-pa-20">
            <ul class="u-list-inline g-color-gray-dark-v6">

              <li class="list-inline-item g-mr-10">
                <a class="u-link-v5 g-color-gray-dark-v6 g-color-lightblue-v3--hover g-valign-middle" href="#!">Kasie</a>
                <i class="hs-admin-angle-right g-font-size-12 g-color-gray-light-v6 g-valign-middle g-ml-10"></i>
              </li>

              <li class="list-inline-item g-mr-10">
                <a class="u-link-v5 g-color-gray-dark-v6 g-color-lightblue-v3--hover g-valign-middle" href="#!">Unit Kerja</a>
                <i class="hs-admin-angle-right g-font-size-12 g-color-gray-light-v6 g-valign-middle g-ml-10"></i>
              </li>

              <li class="list-inline-item">
                <span class="g-valign-middle">Data Unit Kerja</span>
              </li>
            </ul>
          </div>
          <!-- End Breadcrumb-v1 -->
        
          <div class="g-pa-20">
          


            <header class="g-mb-20">
              
               <a href="/kasie/unitkerja/create"><button type="button" class="btn btn-primary">Tambah Unit Kerja</button></a>
            </header>

             @if(session('msg'))

   <div class="alert alert-success">
  <strong> {{session('msg')}}</strong>
</div>

@endif


             @if(session('msg2'))

   <div class="alert alert-danger">
  <strong> {{session('msg2')}}</strong>
</div>

@endif


            <div class="table-responsive g-mb-40">
              <table class="table table-bordered" id="tes">
                <thead>
                  <tr>
                    <th>No</th>
                    <th>Nama</th>
                    <th>Action</th>
                  </tr>
                </thead>

                <tbody>
                	 <?php $no=1; ?>
                @foreach($unit as $r)
            <tr>
         
              <td style="text-align: center">{{$no}}</td>
              <td style="text-align: center">{{$r->nama}}</td>
             
              <td style="text-align: center">
               
                <form action="/kasie/unitkerja/{{$r->id}}" method="post">
              <button type="submit" name="submit" class="btn btn-danger" >Delete</button>
                <input type="hidden" name="_method" value="delete">
                {{csrf_field()}} 
                 <a href="/kasie/unitkerja/{{$r->id}}"><button type="button" class="btn btn-success" title="edit">Edit</button></a>
                 </form>
                  
               </td>
              
            </tr>
            <?php $no++; ?>
            @endforeach
                </tbody>
              </table>
            </div>


@endsection





